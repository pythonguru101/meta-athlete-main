...
use meta_athlete_primitives::{AthleteId, InstanceId};
  ...
  T: Config,
{
  ...
  -> Result<(AthleteId, InstanceId, u32, u32), http::Error> {
    let res = Self::request_response(ORACLE_URL)?;
    Self::parse_athlete_info(&res).ok_or(http::Error::Unknown)
  }

  fn parse_athlete_info(s: &str) -> Option<(AthleteId, InstanceId, u32, u32)> {
